...
let contractRaffleTix = '0xEb240D9DDc9482D3d2BFc75b708BB059ca7188f1';
...
console.log('GROUPS: ', contractAddressGroupsTokens);
...
async function makeUserListing(posterAddress){
    ...
    let nftProfile = await getProfileImageFromNFT(userProfile.avatarMetadata , userProfile.avatarContract, posterAddress, userProfile.groupID);
...
async function makeProfile(userAddress){
    ...
    let nftProfile = await getProfileImageFromNFT(userProfile.avatarMetadata , userProfile.avatarContract, userAddress, userProfile.groupID);
...
async function buildAsGroup(){
    ...
    if (validWalletConnection) {
        ...
        let nftProfile = await getProfileImageFromNFT(userProfile.avatarMetadata , userProfile.avatarContract, walletAddress, userProfile.groupID);
        ...
        if (allGroupIDs.length > 0){
            ...
            for (let g = 0; g < allGroupIDs.length; g++) {
                ...
                let nftProfile = await getProfileImageFromNFT(groupProfile.avatarMetadata , groupProfile.avatarContract, allGroupDetails[g].groupAddress, groupProfile.groupID, true);
